...
interface Props {
  column: { id: KanbanColumnType; title: string; icon: typeof Circle; gradient: string; dotColor: string }
  todos: Todo[]
  isActive?: boolean
}
...
export function KanbanColumn({ column, todos, isActive }: Props) {
  ...
  return (
    <div className="flex w-72 shrink-0 flex-col max-sm:w-[calc(100vw-2rem)]">
      ...
      >
        <div className={`absolute inset-0 bg-gradient-to-b ${column.gradient} opacity-30 transition-opacity duration-300 ${isOver ? "opacity-60" : ""}`} />
        <div className="relative flex flex-col">
          <div className="flex items-center justify-between border-b border-zinc-800/50 px-2.5 py-2 sm:px-3 sm:py-2.5">
            <div className="flex items-center gap-1.5 sm:gap-2">
              <span className={`flex h-2 w-2 ${column.dotColor} shadow-[0_0_6px] shadow-current/50`} />
          ...
          >
            <SortableContext items={todos.map((t) => t.id)} strategy={verticalListSortingStrategy}>
              {todos.length === 0 ? (
                <div className={`flex flex-1 flex-col items-center justify-center border border-dashed py-6 transition-all duration-300 sm:py-10 ${
                  isOver
                    ? "border-purple-500/30 bg-purple-500/5"
                    : "border-zinc-800"
                }`}>
                  <span className="text-[10px] uppercase tracking-widest text-zinc-700">drop here</span>
                </div>
              ) : (
