...
export function KanbanColumn({ column, todos, isActive }: Props) {
  ...
  return (
    <div className="flex w-72 shrink-0 flex-col max-sm:w-[calc(100vw-2rem)]">
      ...
      >
        <div
          className={`absolute inset-0 bg-linear-to-b ${column.gradient} opacity-30 transition-opacity duration-300 ${isOver ? "opacity-60" : ""}`}
        />
        <div className="relative flex flex-col">
          ...
          <div
            ref={setNodeRef}
            className={`flex min-h-30 flex-col gap-1.5 p-1.5 transition-all duration-200 sm:min-h-40 sm:p-2 ${
            ...
            >
              {todos.length === 0 ? (
                ...
                <ScrollArea className="flex-1 -mx-1.5 px-1.5 sm:-mx-2 sm:px-2">
                  <div className="flex flex-col gap-1.5 min-h-25 sm:min-h-30">
                    {todos.map((todo) => (
